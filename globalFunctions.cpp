 ...
 *
 * Parameters:
 * PubSubClient - Initialized client that will be used to publish the message. (I)
 * const char * - Topic to which the message will be published. It's needed in case the client tries to reconnect. (I)
 * const char * - Message that will be published to a topic. It's needed in case the client tries to reconnect. (I)
 * const char * - Client ID.  It's needed in case the client tries to reconnect. (I)
 * const char * - Credentials, username. It's needed in case the client tries to reconnect. (I)
 * const char * - Credentials, password. It's needed in case the client tries to reconnect. (I)
 *
 * Return codes:
 *
 * Description:
 * Published a message to a topic. If it fails, tries to reconnect and tries to publish again. Prints what it's doing on the serial monitor.
 *
 */
